...
## [x] 0.1 Early decisions (do these first)

**Prerequisites:** None.

1. [x] (0.1.1) Telegram client: use `grammers` (MTProto) for the MVP.
   See `docs/adr/20251231-telegram-client-grammers.md`.
2. [x] (0.1.2) TUI framework: use `ratatui`.
   See `docs/adr/20251231-tui-ratatui.md`.
3. [x] (0.1.3) LLM backend: start with OpenAI; store API keys in local `.env`
   for dev-only; implement a proper prod secret store later.
   See `docs/adr/20251231-llm-backend-openai-env.md`.
4. [x] (0.1.4) Data directory: store under the project local folder for
   dev-only; switch to OS-specific dirs later.
   See `docs/adr/20251231-data-dir-local.md`.
5. [x] (0.1.5) LLM-friendly Test Framework: decide on a testing framework
   for this project (favoring deterministic, LLM-friendly tests).
6. [x] (0.1.6) Logging policy: plain logs, size-based rotation, and content
...
1. [x] (1.1.1) Create Cargo workspace with crates: `app` (bin, wiring), `core`
   (Telegram + domain), `ui` (TUI components), `llm` (providers, prompt
   templates), `integration-tests`.
2. [x] (1.1.2) Use mise-en-place to manage tool versions (Rust toolchain, build
   deps, CLI helpers).
3. [x] (1.1.3) Add CI basics: `cargo fmt -- --check`, `clippy -D warnings`,
   `nextest`. Set Rust toolchain in `rust-toolchain.toml`.
4. [x] (1.1.4) Keep Telegram integration grammers-only (MTProto) and document
   any native deps if they appear.
5. [x] (1.1.5) Set up a TUI test harness (headless render/snapshot) and seed
...
1. [x] (2.1.1) Implement client bootstrap (grammers session config, auth flow,
   phone/QR login) and update pump (background async task).
2. [ ] (2.1.2) Model domain events (new message, edited, read receipt, typing)
   and expose as channels or streams to the UI layer.
3. [ ] (2.1.3) Implement send pipeline with rate-limit or backoff handling;
   support text, reply, edit, delete; queue unsent messages when offline.
4. [ ] (2.1.4) Add minimal persistence for chat metadata and message cache to
...
1. [ ] (3.1.1) Layout v1: left chat list, main message view, bottom composer;
   modal for LLM-generated drafts; command palette for actions.
2. [ ] (3.1.2) Input ergonomics: vim or VSCode-style keymaps, scrollback,
   search in chat, message selection for LLM export.
3. [ ] (3.1.3) Accessibility: color themes (light/dark/high-contrast),
   configurable keybindings, resize handling, mouse optional.
4. [ ] (3.1.4) Notifications: status bar for connection state; optional desktop
...
1. [ ] (4.1.1) Export pipeline: select messages -> structured transcript (with
   authors or timestamps) -> send to provider with chosen prompt.
2. [ ] (4.1.2) Draft pipeline: receive LLM draft -> show diff vs last user
   draft -> allow edit -> user explicitly sends.
3. [ ] (4.1.3) Prompt kit: summarize thread, propose reply, extract action
   items, sentiment or priority tagging; keep prompts versioned.
4. [ ] (4.1.4) Safety: truncate or zip transcripts to fit token limits; redact
   secrets before sending; avoid logging auth tokens; log prompts or responses
   for reproducibility (default on).

## 5.1 Tooling, testing, and DX
...
1. [ ] (5.1.1) Unit tests for domain logic (rate limits, message queue);
   integration tests with mocked grammers or recorded sessions.
2. [ ] (5.1.2) Snapshot tests for UI rendering (ratatui) using `insta` with
   deterministic data.
3. [ ] (5.1.3) Tracing or logging with `tracing` + `tracing-subscriber`;
   human-readable logs to `data/logs/app.log`; size-based rotation at 1 MB with
   20 files; toggle verbosity at runtime.
4. [ ] (5.1.4) Developer commands: `cargo xtask dev-env` to run a local config
   wizard and start the app.

## 6.1 Packaging and release
...
1. [ ] (6.1.1) Ship static binaries per target; verify codesign or notarization
   for macOS.
2. [ ] (6.1.2) Provide `.env.example`, a production secret-store helper script
   (e.g., keychain), and minimal onboarding doc (phone login steps, API ID/Hash
   link).
3. [ ] (6.1.3) Optional: publish Homebrew tap and AUR package once MVP
   stabilizes.

## 7.1 Side quests (MCP and tooling)

**Prerequisites:** Core system stable. **Sub-feature ordering:** MCP server
setup -> bench or analysis servers.

1. [ ] (7.1.1) Install helpful MCP servers for the lifecycle: repo or code map
...
2. [ ] (7.1.2) Consider MCP bench or analysis servers to simulate tool-rich
   flows during LLM prompt testing once the core is stable.

## 8.1 MVP acceptance criteria (from SPEC)
...
1. [ ] (8.1.1) User can log in, select a chat, read history, and send a message.
2. [ ] (8.1.2) User can select messages, generate an LLM draft, edit it, and
   send.
3. [ ] (8.1.3) App recovers from temporary disconnect without losing drafts.
...
1. [ ] (9.1.1) M0 (week 1): spikes conclude; stacks chosen; scaffold + CI green.
2. [ ] (9.1.2) M1 (week 2-3): login, chat list, read/send text, basic TUI
   layout.
3. [ ] (9.1.3) M2 (week 4-5): LLM export -> draft -> user send loop; transcript
   saving; keymaps.
4. [ ] (9.1.4) M3 (week 6): resilience (offline queue), theming,
   notifications; first binary release (macOS first).
5. [ ] (9.1.5) M4 (week 7+): polish, packaging, docs, optional plugins
