...
- [x] (2.1.6) Resolve message author display names from cached user profiles
  and non-user author names when provided.

### 2.2 Out of scope

- [ ] (2.2.1) Real-time UI rendering loop or event handling.
- [ ] (2.2.2) User avatar rendering.
...
- [x] (2.3.5) Message author label is `You` for outgoing messages, the message
  author name when provided, a cached user display name when available, or the
  chat title fallback for non-user authors, and `User {id}` otherwise.
...
- [x] (3.1.5) Cache user display names and expose them to UI mapping.
- [x] (3.1.6) Run formatting, linting, and tests.
...
- [x] (4.1.4) Test that cached user display names are used for message authors.
- [x] (4.1.5) Test that chat author names fall back to chat titles.
