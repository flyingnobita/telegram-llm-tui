...
- [x] (2.1.2) Persist recent messages per chat (message_id, author_id,
  author_name, timestamp, edit_timestamp, text, outgoing flag) with author_id
  representing a user or chat peer.
...
- [x] (2.1.8) Persist cached user display names for message authors.
...
- [x] (2.3.1) `ChatSummary` with id, title, peer kind, last message metadata,
  unread count.
- [x] (2.3.2) `CachedMessage` with chat_id, message_id, author_id,
  author_name, timestamp, edit timestamp, text, outgoing.
...
- [x] (2.3.6) `CachedUser` with user_id and display_name for author labels.
...
- [x] (2.4.7) Message events upsert user display names when provided.
...
- [x] (3.1.7) Add cached user profiles and lookup helpers for message authors.
...
- [x] (4.1.6) User display names persist and are available for UI mapping.
