...
- [ ] (2.2.2) Rich media parsing or advanced message formatting beyond
...
- [ ] (2.3.2) `DomainEvent` enum with variants:
  - [ ] (2.3.2.1) `MessageNew` with: `chat_id`, `message_id`, `author_id`,
    `author_name`, `timestamp`, `text` where `author_id` is a user or chat peer.
  - [ ] (2.3.2.2) `MessageEdited` with: `chat_id`, `message_id`, `editor_id`,
    `editor_name`, `timestamp`, `text` where `editor_id` is a user or chat peer.
...
- [ ] (2.4.2) If an update lacks required IDs or timestamps, mapping returns
  `None` and records a mapping error log entry. When `from_id` is missing for
  incoming messages, fall back to the peer chat ID for the author identity.
...
- [ ] (4.1.5) Unit test for channel posts without `from_id` mapping to a chat
  author identity.
