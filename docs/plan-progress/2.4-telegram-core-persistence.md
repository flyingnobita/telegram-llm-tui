...
8. [x] (2.4.1.8) Persist cached user display names for message authors and
   author display names for non-user senders.
...
9. [x] (2.4.2.9) Store user display names and expose lookup helpers for UI mapping.
...
6. [x] (2.4.3.6) User display names are cached and readable for UI mapping.
...
1. (2.4.6.1) Primary log file is `logs/app.log` (configured in
   `app/config/app.toml`).
2. (2.4.6.2) Error log file is `logs/app-error.log` (configured in
   `app/config/app.toml`).
