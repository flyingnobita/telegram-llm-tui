...
1. [x] High: `InputState` treats `cursor` as a character index but uses byte offsets
   ...
   (`ui/src/input.rs:10`)
2. [x] Medium: The global log hotkey intercepts plain `l` before focus handling, so
